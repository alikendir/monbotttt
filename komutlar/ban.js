...
if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send(' Etiketlenen Kullanıcı Sizden Üst/Aynı Yetkide.')
  member.ban({days: 7, reason: reason})

const embed = new Discord.MessageEmbed()
.setTitle(`Banlama İşlemi Tamamlandı`)
    .addField(`Banlayan Yetkili <@${message.author.id}>`) 
    .addField(`Ban Sebepi ${message.author.id}`) 
.setFooter(``)
.setColor('RED')
